...
**未マージ PR からの要約と submodule の扱い**: 最新会合の note がまだ main に無く PR にしかない場合、submodule `raw/notes` で当該 PR を checkout して要約する:

```sh
cd raw/notes && git fetch origin pull/<PR>/head:pr-<PR> && git checkout pr-<PR>
```

- このとき**メインリポジトリの submodule ポインタ変更はコミットしない**(未マージのため、`raw/notes` は working tree 上で modified のまま放置してよい)。要約ファイル(`wiki/meetings/`)だけを `git add wiki/` で commit する。
- **以後の運用**: 定期的に `git submodule update --remote` または `cd raw/notes && git fetch` で更新を確認し、**その PR が main にマージされていたら submodule を main 追跡へ戻す**(`cd raw/notes && git checkout main && git pull --ff-only`)。マージ後は submodule ポインタを通常どおり更新・コミットしてよい(プレフィックスは `[wiki]`)。
