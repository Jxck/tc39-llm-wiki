...
3. ステージ推移グラフ — テーブルの下に mermaid **`gantt`** を埋め込む。**各 Stage を 1 タスク(行ラベル=`Stage N` の文字列)**とし、その stage に居た**期間を横棒**で表す。`dateFormat YYYY-MM` / `axisFormat %Y`(横軸=実時間・年ラベル)/ `todayMarker off`。タスクは stage 遷移順に列挙し、各棒は `Stage N : <到達 YYYY-MM>, <次stage到達 or 現在>` の形。現ステージの棒は現在(直近の `2026-06` 等)まで伸ばす。**撤回・降格も期間で自然に表現**(撤回は最後の棒をその年月で止める。降格は降格後の stage を末尾の棒として置く)。Stage 番号は文字ラベルなので 2.7 もそのまま出る(数値軸ではないので 0.5 刻みは出ない)。グラフ直後に読み方の注記(各遷移の年月)を `>` で添える。例:
   ...
   注: `gantt` は mermaid の標準図種で VSCode 既定の `bierner.markdown-mermaid` でも描画される(旧 `xychart-beta` は同拡張で空描画だった)。title は ASCII 推奨。棒の長さで「どの stage に何年いたか」が読め、正確な遷移月はステージ遷移テーブルと `>` 注記で併記する(目盛りは年単位の等間隔グリッド)。
