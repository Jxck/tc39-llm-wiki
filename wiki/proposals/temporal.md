...
| [2025-04](../../raw/notes/meetings/2025-04/april-14.md) | Firefox 139 で出荷予定と報告。「Stage 3 のまま出荷可能」                                              | 3 (維持)     |
| [2026-03](../../raw/notes/meetings/2026-03/march-11.md) | Stage 4 到達。仕様文書の分離・専用 TG は当面見送り                                                    | 3 → 4        |

```mermaid
gantt
    title Temporal stage timeline
    dateFormat YYYY-MM
    axisFormat %Y
    todayMarker off
    Stage 1 : 2017-03, 2018-09
    Stage 2 : 2018-09, 2021-03
    Stage 3 : 2021-03, 2026-03
    Stage 4 : 2026-03, 2026-06
```

> 各 Stage の横棒 = その stage に居た期間(横軸 = 実時間)。Stage 1 (2017-03) → Stage 2 (2018-09) → Stage 3 (2021-03) → Stage 4 (2026-03)。Stage 3 に約 5 年とどまった長さが見て取れる。Temporal は Stage 2.7 制度 (2023 新設) より前に 2 → 3 を遷移したため 2.7 は経ていない。
...
Temporal の Stage 2 と同じ 2018-09 会合の**別アジェンダ「IDL for JavaScript」**(presenter [DE](../people/DE.md))での議論です。Temporal 自体の審議ではないが、Temporal のような大きな API をどの IDL で記述するか (WebIDL を使うか、JS 向けの "JSIDL" を作るか) という形で関連しました。[WH](../people/WH.md) は「IDL の仕様自体が複雑で、外部標準に依存すると ECMAScript 仕様との間で循環参照のリスクがある」と懸念。[JHD](../people/JHD.md) は「IDL に合わせるためだけに 3 万行規模の normative change を入れてレビューするのは困難」と反対し、段階的に進める方針となりました。
...
2024-06 が最大の論点で、[JGT](../people/JGT.md) は「Google・Apple・Mozilla から Temporal は大きすぎるという強いフィードバックを受けた。圧力は ECMAScript の外、Apple Watch や Android など低スペック端末を担当する人々から来ている」と説明しました。[SYG](../people/SYG.md) は V8 を代表して prepared statement を述べています:

> ([SYG](../people/SYG.md)) V8 はこの scope reduction を強く支持し、終盤での簡素化にこれほど前向きであった champion たちに感謝する。
...
- [Intl Era/Month Code](../proposals/intl-era-month-code.md) — era / month code の locale 依存挙動。ECMA-402 側で Temporal と並行して標準化され、2026-03 に Stage 4 到達。
...
- [2025-04 april-14](../../raw/notes/meetings/2025-04/april-14.md) — Firefox 139 出荷予定、「Stage 3 のまま出荷可能」と報告
